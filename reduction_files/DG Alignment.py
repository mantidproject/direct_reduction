...
#==================================================#
#           Runs + Peaks                              #
run_numbers = [""] #input as a list eg. [“10001”,”10003”,”10004”]

corresponding_hkls = [] # please write in same order the runs are written in
# e.g[[h1,k1,l1],[h2,k2,l2]]

peak_picking_method =  #Peak_picking_methods.{METHOD}
# Peak_picking_methods.MANUAL or Peak_picking_methods.ROI


UB_method =  #UB_METHOD.{CHOSEN_METHOD} E.G UB_METHOD.ONE_PEAK Available methods:ONE_PEAK, TWO_PEAKS, THREE_PEAKS, FIVEPLUS_PEAKS
...
Reorient = #Enter True or False
hkl_of_desired_alignment= [,] # please input as a single peak: [h,k,l]

Predict_peaks = #Enter true or false
min_dspacing = #in A^-1
max_dspacing = #in A^-1

unit_cell_space_group = #this either needs to be a string of the symbol e.g "F d -3 m" or and integer with no "",
# if you don't know your space group number enter "N/A"
unit_cell_symmetry = reflection_condition.PRIMITIVE
...
#===============================#
#   Staff Input
instrument_name =  #Enter instrument name in all caps, example: MAPS
cycle = '2025_1'
rbnum = ''


cycle_shortform = cycle[2:] if cycle.startswith('20') else cycle
datadir = f'/data/instrument/{instrument_name}/CYCLE20{cycle_shortform.replace("_","")}/RB{rbnum}/'


d_tolerance = #Tolerenace on peak position, in d-spacing in A^-1
hkl_tolerance = #Used in UB quality validation, for how close indexes calculated have to be to whole
...
match peak_picking_method:
    case Peak_picking_methods.MANUAL:
        for x in range(len(run_numbers)):
            if instrument_name == LET:
                ws = Load(str(datadir + instrument_name.i_name[0:3] +"000"+ run_numbers[x]), EnableLogging=False)
            else:
                ws = Load(str(datadir+instrument_name.i_name[0:3]+run_numbers[x]),  EnableLogging=False)
            ...
            SetGoniometer(ws,**instrument_name.axes,  EnableLogging=False) # ** unravels dictionary containing axes, + sets goniometer on each workspace
...
##ROI + pick best pixel
    case Peak_picking_methods.ROI:
        for x in range(len(run_numbers)):
            ...
            ws = ConvertUnits(ws, Target='dSpacing', EMode='Elastic', EFixed=None,  EnableLogging=False)#converting TOF to dspacing
            SetGoniometer(ws, **instrument_name.axes, EnableLogging=False)
...
match UB_method:
    ...
    case _:
        raise ValueError("Method Entered doesn't exist")

if 2<=UB_method.value<= 4:
    Number_of_peaks = SingleCrystalPeakTable.getNumberPeaks()
...
if Reorient == True:
    ...
    data_file.write(f"{ct}\n UB Calculation Method: {UB_method}\n\nUsed Peaks: \n{used_peaks}\nUB From Peak Data:\n {np.round(UB_unaligned,4)},\n\n u vector: {np.round(u_unaligned,4)},\n v vector:{np.round(v_unaligned,4)},\n\nTo align q = {hkl_of_desired_alignment} along beam direction:\n\nRotate to Psi0 {theta_user},\n\n Using {Axis1} as second Axis \n\nThe offset from the plane of the provided q is: {Phi} \n\nUB After Psi Rotation:\n {np.round(UB_final,4)},\n\n u vector: {np.round(u_aligned,4)},\n v_vector: {np.round(v_aligned,4)} ")
...
if Predict_peaks == True:
    ...
    ws = Load(str(datadir+instrument_name.i_name[0:3]+run_numbers[0]), EnableLogging=False)
    SetGoniometer(ws, **instrument_name.axes, EnableLogging=False)
